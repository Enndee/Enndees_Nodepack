/**
 * VideoFrameExtractor â€“ ComfyUI custom widget  (v3)
 *
 * Changes vs v2.1:
 *  â€¢ Playhead removed entirely â€” loop region is the only scrub mechanism.
 *  â€¢ Hoverable loop zone: handles glow on hover; body brightens with a
 *    dashed highlight border so the user knows what is grabbable.
 *  â€¢ Transport bar with large Play / Pause / Stop buttons drawn in the
 *    widget itself, positioned between the preview and the timeline.
 *  â€¢ rAF loop drives videoEl.currentTime through the loop window.
 */
...
const RESIZE_H = 14; // preview resize handle â€” tall enough to be obvious
const PREVIEW_H_DEFAULT = 160;
const PREVIEW_H_MIN = 80;
const PREVIEW_H_MAX = 400;
// WIDGET_H is dynamic â€” computed in widget.computeSize()
...
const C = {
  ...
  // Playback position line (replaces playhead â€” simpler, no dragging)
...
// â”€â”€â”€ Widget factory â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function createVideoTimelineWidget(node, videoWidgetName) {
  ...
  // â”€â”€â”€ Cursor â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  let _canvasEl = null,
    _lastCursor = "";
  ...
  // â”€â”€â”€ Transport / playback â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  function stop() {
    // Internal reset â€” called when video changes or loop moves.
  ...
  function stepFrame(delta) {
    ...
    loopIn = newIn;
    loopOut = newOut;
    currentFrame = lastMoved === "out" ? loopOut : loopIn;
    if (videoEl && fps > 0) videoEl.currentTime = currentFrame / fps;
    syncWidgets();
    node.setDirtyCanvas(true, false);
  }
  ...
  async function fetchVideoInfo(videoName) {
    if (!videoName) return;
    // Halt any playback/looping on the previous video immediately â€” before
    // we await anything â€” so the rAF loop can't race with the new src load.
    cancelAnimationFrame(rafId);
    rafId = null;
    playState = "paused";
    if (videoEl) {
    ...
    try {
      const r = await fetch(
        `/enndee_video_frame_extractor/info?video=${encodeURIComponent(videoName)}`,
      );
      ...
      node.setDirtyCanvas(true, false);
      console.warn("[Enndee VFE+ Set] fetchVideoInfo:", e);
    }
  }
  ...
  function loadVideoEl(videoName) {
    ...
    if (!videoEl) {
      ...
      videoEl.addEventListener("loadedmetadata", () => {
        loadStage = "buffering";
        lastStageEvent = `Metadata parsed (${videoEl.videoWidth}Ã—${videoEl.videoHeight})`;
        node.setDirtyCanvas(true, false);
      });
      ...
      videoEl.addEventListener("error", () => {
        ...
        clearTimeout(loadTimeoutId);
        console.warn("[Enndee VFE+ Set] video load error", videoEl.error);
        node.setDirtyCanvas(true, false);
      });
    ...
    if (videoEl.dataset.src !== url) {
      ...
      // Watchdog â€” if loading never completes, flag it as a timeout
      clearTimeout(loadTimeoutId);
      loadTimeoutId = setTimeout(() => {
        if (!videoReady) {
          videoError = "Load timed out â€” video may be corrupt or unreadable";
          node.setDirtyCanvas(true, false);
        }
      }, LOAD_TIMEOUT_MS);
      ...
      // Same URL, already loaded â€” just mark ready again
      videoReady = true;
      videoLoadProgress = 1;
      node.setDirtyCanvas(true, false);
    }
    ...
    for (let b = 0; b < indices.length; b += BATCH) {
      await Promise.all(
        indices.slice(b, b + BATCH).map(async (fi, bi) => {
          try {
            const url = `/enndee_video_frame_extractor/thumbnail?video=${encodeURIComponent(videoName)}&frame=${fi}`;
            const img = new Image();
  ...
  // â”€â”€â”€ Draw helpers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  function drawPreview(ctx, x, y, w, h) {
    ...
    if (videoEl && videoReady) {
      ...
      ctx.fillText("âš   Video error", cx, cy - 22);
      ...
      // Video is loading â€” show detailed status
      ...
      else if (loadStage === "fetching-info") headline = "Fetching video infoâ€¦";
      else if (loadStage === "loading-metadata") headline = "Loading metadataâ€¦";
      else if (loadStage === "buffering") headline = pct > 0 ? `Buffering ${pct}%` : "Bufferingâ€¦";
      else headline = "Loading videoâ€¦";
      ...
      if (videoLoadProgress > 0) {
        ...
        // Indeterminate shimmer â€” a sliding block
      ...
      if (parts.length) {
        ...
        ctx.fillText(parts.join("   â€¢   "), cx, cy + 28);
      }
    } else {
      ctx.fillStyle = C.textDim;
      ctx.font = "12px 'Courier New', monospace";
      ctx.textAlign = "center";
      ctx.fillText("â¬†  Upload a video above", x + w / 2, y + h / 2 + 4);
    }
    ...
    if (totalFrames > 0) {
      ...
      if (vidW && vidH) {
        ...
        ctx.font = "10px 'Courier New', monospace";
        ctx.textAlign = "right";
        ctx.fillText(`${vidW}Ã—${vidH}`, x + w - 6, y + 14);
      }
    }
  }

  // â”€â”€ Transport bar â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  // Layout (left â†’ right):
  //   [â® JUMP BACK] [â—€ PREV] [â–¶ PLAY / â¸ PAUSE] [NEXT â–¶] [JUMP FWD â­]
  //
  ...
  function drawTransport(ctx, x, y, w, h) {
    ...
    buttons.push({
      id: "btnPlayPause",
      label: isPlaying ? "â¸  PAUSE" : "â–¶  PLAY",
    ...
    // Progress bar â€” slim strip at bottom of transport row
    if (totalFrames > 0) {
      const pbY = y + h - 5;
      const pbW = w - 20;
      const pbX = x + 10;
  ...
  function drawRuler(ctx, x, y, w, h) {
    ...
    // Hover tint â€” signals the ruler is clickable
  ...
  function drawFilmstrip(ctx, x, y, w, h) {
    ...
    if (!totalFrames || thumbCache.size === 0) {
      ...
      ctx.textAlign = "center";
      ctx.fillText("Loading filmstripâ€¦", x + w / 2, y + h / 2 + 3);
      return;
    }
  ...
  // â”€â”€ Loop bar â€” the star of the show â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  function drawLoopBar(ctx, x, y, w, h) {
    ...
    // â”€â”€ Body fill â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ctx.fillStyle = anyHov ? C.loopFillHov : C.loopFill;
    ctx.fillRect(lx, y, regionW, h);

    // â”€â”€ Body border â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    if (anyHov) {
    ...
    // â”€â”€ "Drag me" hint label inside body â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    if (regionW > 80 && !anyHov) {
      ctx.fillStyle = "rgba(99,102,241,0.4)";
      ctx.font = "9px 'Courier New', monospace";
      ctx.textAlign = "center";
      ctx.fillText("â†” drag", lx + regionW / 2, y + h * 0.68);
    }

    // â”€â”€ Frame count label â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    if (regionW > 50) {
    ...
    // â”€â”€ Playback position line inside loop â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ Handles â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  function drawCollapseToggle(ctx, x, y, w, h) {
    ...
    if (collapsed && totalFrames > 0) {
      ctx.textAlign = "right";
      ctx.fillText(
        `${totalFrames} fr  â€¢  ${fps.toFixed(0)} fps`,
        x + w - 4,
        y + h / 2 + 4,
      );
    }
  ...
  function drawResizeHandle(ctx, x, y, w, h) {
    ...
    // Background â€” lightens on hover to signal interactivity
    ...
    // Label â€” centred vertically
    ctx.fillStyle = isHov ? "#c7d2fe" : "#55556a";
    ctx.font = "9px 'Courier New', monospace";
    ctx.textAlign = "center";
    ctx.fillText("â†•  DRAG TO RESIZE", x + w / 2, y + h / 2 + 4);
  }
  ...
  function drawInfoBar(ctx, x, y, w) {
    ...
    const stateLabel = playState === "playing" ? "â–¶ PLAYING" : "â¸ PAUSED";

    // Row 1 â€” loop summary
    ctx.font = "bold 13px 'Courier New', monospace";
    ctx.textAlign = "left";
    ctx.fillStyle = "#ffffff";
    ctx.fillText(
      `LOOP  ${loopIn} â†’ ${loopOut}  (${nFrames} fr / ${dur}s)   â€¢   ${stateLabel}`,
      x + 2,
      y + 14,
    );
    const vS = viewStart,
    ...
    ctx.font = "12px 'Courier New', monospace";
    ctx.fillText(`zoom ${zoomPct}%  â‡§+scroll=pan`, x + w - 2, y + 14);

    // Row 2 â€” memory estimate (color-coded)
    if (bytesPerFrameF32 > 0) {
      ...
      let color;
      if (ratio >= 1) color = "#ef4444"; // red â€” will be blocked
      else if (ratio >= 0.75) color = "#f59e0b"; // amber â€” risky
      else color = "#86efac"; // green â€” safe

      const icon = ratio >= 1 ? "â›”" : ratio >= 0.75 ? "âš " : "âœ“";
      const statusText =
        ratio >= 1
          ? "WILL BE BLOCKED"
          : ratio >= 0.75
      ...
      ctx.fillText(
        `${icon} MEM est. ${formatBytes(peak)} / ${formatBytes(memoryLimitBytes)} limit  â€¢  ${statusText}`,
        x + 2,
        y + 30,
      );
      ...
      ctx.font = "11px 'Courier New', monospace";
      ctx.fillText(
        `${nFrames}+${customN} fr @ ${vidW}Ã—${vidH}`,
        x + w - 2,
        y + 30,
      );
    }
  ...
  // â”€â”€â”€ Widget object â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const widget = {
    ...
    setLoop(newIn, newOut) {
      if (!totalFrames) return;
      const prevMoved = lastMoved;
      loopIn = Math.max(0, Math.min(newIn, totalFrames - 1));
      loopOut = Math.max(loopIn, Math.min(newOut, totalFrames - 1));
      if (prevMoved === "out") {
        currentFrame = loopOut;
      } else if (prevMoved === "body") {
        currentFrame = loopIn + Math.round((loopOut - loopIn) / 2);
      } else {
        currentFrame = loopIn;
      }
      if (videoEl) {
    ...
    draw(ctx, nodeObj, widgetWidth, y) {
      ...
      // Collapse toggle row â€” always visible
      const collapseY = y + 4;
      drawCollapseToggle(ctx, iX, collapseY, iW, COLLAPSE_H);

      // Sections below â€” heights vary when preview is collapsed
    ...
    mouse(event, pos) {
      ...
      // â”€â”€ Wheel â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      if (event.type === "wheel") {
      ...
      // â”€â”€ Pre-compute zone membership â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      const inCollapse =
      ...
      // â”€â”€ Pointer down â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      ...
      // â”€â”€ Pointer move â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      if (event.type === "pointermove") {
        ...
        // Hover detection â€” exactly one zone wins
        let hov = null;
        let cur = "default";
        if (inCollapse) {
          hov = "collapse";
      ...
      // â”€â”€ Pointer up â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€â”€ Drag targeting â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  function applyDrag(mx, tX, tW) {
    ...
    // Always pause; seek the preview to the frame of the marker that was
    // moved last (start, end or body) so the user sees what they adjust.
    cancelAnimationFrame(rafId);
    rafId = null;
    playState = "paused";
    if (drag.type === "loopOut") {
      lastMoved = "out";
    } else if (drag.type === "loopIn") {
      lastMoved = "in";
    } else {
      lastMoved = "body";
    }
  ...
  // â”€â”€â”€ Poll for video widget changes â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  let lastVideo = null;
  setInterval(() => {
...
app.registerExtension({
  name: "Comfy.EnndeeVideoFrameExtractorWithAudio.v1",

  async beforeRegisterNodeDef(nodeType, nodeData) {
    if (nodeData.name !== "Enndee_VideoFrameExtractorWithAudio") return;
    ...
    nodeType.prototype.onNodeCreated = function () {
      ...
      // num_frames: auto-computed from loop span â€” hide it completely so the
      ...
      // Watch start_frame and end_frame spinners â€” sync loop region when changed
